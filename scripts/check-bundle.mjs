...
if (codeKb > CODE_LIMIT_KB + TOLERANCE_KB) {
	fail(`код ${codeKb.toFixed(1)} КБ понад стелю ${CODE_LIMIT_KB} КБ (${heaviest.page})`);
}
if (heaviest.kb > TOTAL_LIMIT_KB + TOLERANCE_KB) {
	fail(
...
if (problems.length > 0) {
	 ...
	 * чекають люди. Бюджет розміру відповідає на «чи продукт не гладшає» — це
	 * тренд, а не питання цілості.
	 *
