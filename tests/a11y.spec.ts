...
async function waitForTitleShown(page: Page) {
	// `h1` є не на кожній сторінці цього проєкту (головна побудована на секціях
	// без наскрізного заголовка), тож умова застосовується лише там, де він є.
	// Без цієї перевірки помічник падав із «element(s) not found» — тобто гейт
	// червонів через власне припущення, а не через доступність.
...
test('головна сторінка не має машинно-виявних порушень WCAG', async ({ page }) => {
	await page.goto('/DigitalWorkshop/');
