...
export class ThemeState {
    ...
    private apply(theme: ThemeType) {
        ...
        if (browser) {
             ...
             * і прибраний він з двох причин, обидві перевірені в браузері.
             *
             * 1. ВІН БУВ ПРОСТО НЕПРАВИЛЬНИЙ ДЛЯ `colorful`. Кольорова тема
             *    світла (`#f5f5f7` тло, `#1a1a1a` текст), а тернар відносив до
             *    `light` лише саму `light` — тобто на світлій сторінці браузер
             *    вважав схему темною. Наслідок видно там, де малює браузер, а не
             *    ми: нативні смуги прокрутки, поля вводу, `<select>`, автозаповнення
             *    — темні елементи на світлому тлі. Заодно він суперечив
             *    мета-тегу рядком вище, який для `colorful` віддає `light dark`.
             *
             * 2. ІНЛАЙН-СТИЛЬ ПЕРЕБИВАЄ ВСІ ТАБЛИЦІ. Схема тепер оголошена в
             *    `app.css`: `:root { color-scheme: light dark }` плюс звуження
             *    `html[data-theme=...]` під кожну тему. Інлайн-значення робило ці
             *    правила недосяжними — заміряно: при `data-theme="light"`
             *    обчислений `color-scheme` лишався `dark`.
             *
             * Атрибут `data-theme` вище — єдине, що тепер потрібно: схему з нього
             * виводить CSS, і робить це однаково для всіх трьох тем.
             */
        }
    }
