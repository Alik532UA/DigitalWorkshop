<script lang="ts">
     ...
     * коли розкривач відкритий, тож три способи зв'язку з чотирьох були
     ...
     *
     * ## Чому пішов і `role="button"`
     *
     * Міркування вище правильне щодо `<button>` — і рівно тією ж мірою стосується
     * РОЛІ. `role="button"` разом із `tabindex="0"` відтворював ту саму проблему
     * на рівні ARIA: елемент оголошував себе кнопкою й містив focusable `<a>`.
     * axe називає це `nested-interactive` (serious, WCAG 4.1.2) і знайшов рівно
     * два таких вузли першим прогоном гейта 2026-08-23.
     *
     * Наслідок для читача екрана був не теоретичний: кнопка, що містить
     * посилання, віддається як один вузол із невизначеною поведінкою, і
     * пробитися до самого посилання буває неможливо.
     *
     * Разом із роллю пішов і `aria-expanded`: на елементі без ролі (тобто
     * `generic`) цей атрибут заборонений специфікацією, і axe називає це
     * `aria-allowed-attr`. Тобто зняти лише роль було недосить — і другий прогін
     * гейта це показав одразу, замість того щоб лишити напівзроблену правку.
     *
     * ЦІНА, і вона справжня: стан «розкрито / згорнуто» більше не оголошується
     * читачем екрана. Додаткові способи зв'язку при цьому досяжні — вони
     * з'являються у фокусі й у порядку обходу, — але про сам факт розкриття
     * повідомлення немає. Правильний повний розв'язок — окрема справжня
     * `<button aria-expanded>` ПОРУЧ із посиланням, а не навколо нього; це
     * зміна розмітки в місцях виклику, і вона свідомо лишена окремою задачею.
     * Пункт про це стоїть у чеклисті бета-тестування.
     *
     * Тепер обгортка тригера — звичайний контейнер без ролі й без власної
     * зупинки в порядку обходу. Клавіатурний шлях від цього не втратив нічого:
     * фокус іде ПРЯМО на підставлене посилання (воно й було головною дією), а
     * `onkeydown` лишився на контейнері — події з посилання доходять до нього
     * бульбашкою, тож Enter і Space так само розкривають решту способів зв'язку.
     * Escape і вихід фокуса працюють як раніше.
     *
    ...
    function closeAndReturnFocus() {
        if (!isHovered) return;
        isHovered = false;
        // Фокус повертається на ПОСИЛАННЯ всередині тригера, а не на сам тригер:
        // після зняття `role="button"`/`tabindex` контейнер більше не focusable,
        // і `focus()` на ньому тихо не робив би нічого — фокус лишався б на
        // зникаючій іконці, тобто «фокус зник» для того, хто ним користується.
        requestAnimationFrame(() => {
            const target =
                triggerEl?.querySelector<HTMLElement>('a, button, [tabindex]:not([tabindex="-1"])') ??
                triggerEl;
            target?.focus();
        });
    }
    ...
    {#if isHovered}
        <div class="social-options" transition:fly={{ 
            ...
            <!-- `loading="eager"` названо явно, а не пропущено: значки існують у
                 DOM лише поки розкривач відкритий, тобто з'являються вже після
