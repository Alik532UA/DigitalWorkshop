...
/**
 * Текст над відео не бере колір із теми (UI-UX-v8 § 1.5.1, `--text-primary`).
 *
 * Сторінка моря лежить ПОВЕРХ відео на весь екран. Тла теми там не видно
 * взагалі, тож `--text-primary` — колір, розрахований на це тло, — тут не
 * означає нічого. А спадковим коренем для всієї сторінки лишався
 * `body { color: var(--text-primary) }` з `app.css`.
 *
 * Наслідок заміряно на живому пристрої: `.project-desc` у картці проєкту не
 * оголошував власного кольору, успадковував `light-dark(#1d1d1f, #f5f5f5)` — і
 * у СВІТЛІЙ системній темі абзац виходив майже чорним на морі. Сусіди в тій
 * самій картці (`h3`, `.project-feature`, кнопка) свій білий оголошують, тож
 * чорнів рівно один абзац із чотирьох — у звіті це й прозвучало як «частина
 * тексту чорного кольору».
 *
 * ЦЕ НЕ ДЕФЕКТ APPLE, хоча знайдений там. Те саме дає будь-який пристрій зі
 * світлою системною темою; на машині розробника тема темна, і `--text-primary`
 * віддає `#f5f5f5` — тобто випадково правильний колір. Клас
 * AI-AGENT-PITFALLS-v8 § 1.4: дефект живе рівно там, куди розробник не дивиться.
 *
 * Перевірка тримає ОБИДВА боки:
 *  1. `.sea-container` оголошує власний `color` — тоді елемент, який свій колір
 *     забув, падає на білий контейнера, а не на тему;
 *  2. у піддереві над відео немає жодного посилання на текстові токени теми —
 *     інакше пункт 1 обходиться однією декларацією.
 */
describe("текст над відео не успадковує тему", () => {
	const OVER_VIDEO = [
		"src/routes/[[lang=lang]]/+page.svelte",
		...readdirSync(join(ROOT, "src/lib/components/sea")).map((f) => `src/lib/components/sea/${f}`)
	].filter((f) => f.endsWith(".svelte"));

	const read = (f: string) => readFileSync(join(ROOT, f), "utf8");

	it("перевірка жива: файли над відео знайдено", () => {
		expect(OVER_VIDEO.length, "піддерево сторінки моря порожнє — перевіряти нічого").toBeGreaterThan(1);
	});

	it(".sea-container оголошує власний color", () => {
		const page = withoutComments(read(OVER_VIDEO[0]));
		const rule = /\.sea-container\s*\{([^}]*)\}/.exec(page);
		expect(rule, "правила .sea-container більше немає — перевірка втратила предмет").not.toBeNull();
		expect(
			/(^|[;\s])color\s*:/.test(rule![1]),
			"без власного color спадковим коренем сторінки моря знову стає " +
				"body { color: var(--text-primary) } — у світлій темі це чорний текст на морі"
		).toBe(true);
	});

	it("жоден файл над відео не посилається на текстові токени теми", () => {
		const bad: string[] = [];
		for (const file of OVER_VIDEO) {
			for (const m of withoutComments(read(file)).matchAll(/var\(\s*(--text-(?:primary|secondary))/g)) {
				bad.push(`${file}: ${m[1]}`);
			}
		}
		expect(
			bad,
			"тло теми під цим піддеревом не видно — колір, розрахований на нього, " +
				`тут випадковий:\n${bad.join("\n")}`
		).toEqual([]);
	});
});
