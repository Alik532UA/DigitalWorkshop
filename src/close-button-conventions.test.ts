...
describe('кнопки закриття (UI-ELEMENTS-v8 § 3)', () => {
	...
	/**
	 * Канарка навмисно слабша, ніж мала б бути, і ось точна причина.
	 *
	 * До 2026-08-15 тут стояло `expect(tags.length).toBeGreaterThan(0)` — щоб
	 * зміна локатора не перетворила перевірку на тихий зелений нуль. Того дня
	 * `ui/BaseModal.svelte` видалили як недосяжний (PROJECT-STRUCTURE-v8 § 4.3),
	 * і разом із ним із проєкту пішла ЄДИНА кнопка закриття. Селектор при цьому
	 * цілий: зник предмет, а не перевірка.
	 *
	 * Тому лишається сканер: він доводить, що файли читаються. Три перевірки
	 * нижче чекають на першу ж модалку й спрацюють на ній.
	 *
	 * ЩОЙНО В ПРОЄКТІ З'ЯВИТЬСЯ КНОПКА ЗАКРИТТЯ — поверни сильний рядок:
	 *     expect(tags.length, 'жодної кнопки закриття не знайдено').toBeGreaterThan(0);
	 * Без нього наступне зникнення локатора не помітить ніхто.
	 */
	it('сканер живий — .svelte-файли читаються', () => {
