...
<style>
	...
	.sea-container {
		...
		/*
		 * Базовий колір тексту для ВСЬОГО, що лежить над відео.
		 *
		 * Без нього спадковим коренем лишався `body { color: var(--text-primary) }`
		 * з `app.css` — тобто `light-dark(#1d1d1f, #f5f5f5)`. Будь-який елемент
		 * тут, який забув оголосити свій колір, отримував у СВІТЛІЙ темі майже
		 * чорний текст на морі. Один такий уже був: `.project-desc`.
		 *
		 * Це не дублювання: кожен видимий елемент над відео колір оголошує сам
		 * (заміряно — токенів теми в цьому піддереві немає жодного), тож рядок
		 * нічого не перефарбовує. Він прибирає САМУ МОЖЛИВІСТЬ падіння на тему
		 * там, де тла теми не видно взагалі.
		 */
		color: white;
		overflow: hidden;
		user-select: none;
	...
	/*
	 * Колір ТУТ обов’язковий, і його відсутність була дефектом, а не стилем.
	 *
	 * Ця картка стоїть НАД ВІДЕО, а не над тлом теми. Без власного `color`
	 * абзац успадковував `body { color: var(--text-primary) }` з `app.css`, а
	 * це `light-dark(#1d1d1f, #f5f5f5)` — тобто у СВІТЛІЙ темі майже чорний
	 * текст на морі. Сусіди в тій самій картці (`h3`, `.project-feature`,
	 * кнопка) свій білий оголошують, тож чорнів рівно один абзац із чотирьох:
	 * саме так це й виглядало у звіті — «частина тексту чорного кольору».
	 *
	 * На машині розробника з темною системною темою `--text-primary` віддає
	 * `#f5f5f5`, і дефекту не видно взагалі. Тому це не «баг Apple»: те саме
	 * дає будь-який пристрій зі світлою системною темою.
	 *
	 * Значення взяте з `LeftCarousel.svelte`, де та сама картка малюється
	 * другий раз і де цей рядок не загубився.
	 */
	.project-desc {
	...
	@media (max-width: 768px) {
		...
		.sidebar-icons {
			...
			/*
			 * Проміжок плинний, і числа в ньому не декоративні (FLUID-SIZING-v9
			 * § 2: розмір від екрана через `clamp`, а не другий брейкпойнт).
			 *
			 * WCAG SC 1.4.10 вимагає 320 CSS px без горизонтального гортання
			 * (ACCESSIBILITY-v9 § 10.5 `A11Y-REFLOW`). Заміряно 2026-09-11
			 * Playwright'ом над ЗІБРАНИМ сайтом: при 320 px група керування
			 * важила 328 px і вилазила на 6 px ліворуч і 4 px праворуч — дев'ять
			 * елементів, серед них активна кнопка вкладки. Смуги прокрутки при
			 * цьому НЕ БУЛО (`scrollWidth === clientWidth === 320`), тобто
			 * відрізане не гортається, а недосяжне зовсім.
			 *
			 * П'ять вкладок дають чотири проміжки, тож потрібні 8 px економії
			 * знімаються звуженням кожного з 8 до 4 px. `20vw - 60px` — пряма
			 * через дві заміряні точки: 4 px при 320 і 8 px при 340. Верхня межа
			 * `clamp` тримає колишні 8 px на кожній ширині від 340 і вище, тож
			 * жоден поширений телефон (360, 375, 390, 414) не змінюється ні на
			 * піксель — це перевірено тим самим прогоном.
			 *
			 * Другим медіа-запитом це не зроблено свідомо: файл стоїть рівно на
			 * своїй стелі в `OVERSIZED` (`src/structure.test.ts`), і одинадцять
			 * доданих рядків CSS її пробивали. Ратчет не піднімають — знаходять
			 * спосіб не рости.
			 */
			gap: clamp(0.25rem, 20vw - 60px, 0.5rem);
		}
