...
export const handleError: HandleClientError = ({ error, event, status }) => {
	...
	/*
	 * Порожній рядок НАВМИСНО, а не недогляд. `+error.svelte` показує власний
	 * локалізований текст (`text.genericMessage`), а `error.message` виводить
	 * окремим рядком «деталі» — і лише коли він непорожній. Підставити сюди
	 * текст рантайму означало б показати відвідувачу нутрощі застосунку;
	 * підставити ще одне узагальнене речення — надрукувати два рядки про те
	 * саме.
	 */
	return { message: '' };
};
