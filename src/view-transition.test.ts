...
describe('перевірка жива', () => {
	 ...
	 *
	 * Умова названа КОНКРЕТНИМ маркером, а не «файл непорожній»: `src/lib/index.ts`
	 * складається з одного коментаря-заглушки SvelteKit і після прибирання
	 * порожній ЗАКОННО. Гейт, який червоніє на цьому, довелося б послабити — а
	 * послаблена перевірка живості не перевіряє нічого.
	 */
