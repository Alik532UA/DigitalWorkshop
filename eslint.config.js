...
export default ts.config(
	...
	...svelte.configs['flat/recommended'],
	{
		...
		rules: {
			...
			//
			// ЧИСЕЛ ТУТ БІЛЬШЕ НЕМАЄ, і це не спрощення. Вони стояли в цих
			// коментарях і двічі розійшлися з дійсністю: спершу на двадцять
			// (обіцяли 87 при реальних 67), потім на одиницю (17 при 16 у
			// `no-unused-vars`). Коментар не виконується, тож старіє саме тоді,
			// коли робота йде добре (AI-AGENT-PITFALLS-v8 § 5.5).
			//
			// Тепер борг живе однією мапою `DEBT` у `src/eslint-baseline.test.ts`,
			// і гейт звіряє її з прогоном ESLint на кожному `npm run test:unit` —
			// в обидва боки: і коли борг виріс, і коли число застаріло після
			// виправлення. Тобто число нарешті отримане командою, а не з пам'яті.

			// Службові імена (`_`, `_e`) боргом не рахуються — див. шаблони нижче.
			...
			// SEO-v8 § 1.5. resolve() типізований проти списку реальних
			...
			// CODE-QUALITY-v8 § 1, HIGH. Один випадок уже прибрано — `error: any` у
			// `ErrorFallback` замінено на `unknown`.
	...
	},
	{
		 ...
		 */
		ignores: ['build/', '.svelte-kit/', 'dist/', '.private/', '.claude/']
	},
